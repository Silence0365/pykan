...
MODEL_CKPT = './QAT_KAN/checkpoints/QAT_model.pth'                              # 模型路径
...
# ==============================
print("Loading data...")
input_coord = np.load(INPUT_COORD_PATH)   # (N, 3)
true_energy = np.load(ENERGY_PATH)      # (N,)
assert len(input_coord) == len(true_energy), "Mismatch in data length!"
...
for sample in in_test:
    idx = np.where((input_coord == sample).all(axis=1))[0]
    if len(idx) > 0:
...
assert np.allclose(in_test, input_coord[test_indices]), "Test set alignment failed!"
