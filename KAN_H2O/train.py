...
model = KAN(width=[3,10,1], grid=10, k=3, seed=42, device=device)

# data load
input_dir = "MD/process_data/descriptor.npy"
...
#training
results = model.fit(dataset, opt="LBFGS", steps=200,lamb=0.0001);#model.fit中具有返回值定义——自行跳转
...
# #training
# results = model.fit(dataset, opt="LBFGS", steps=50,lamb=0);
# train_losses += results['train_loss']
# test_losses += results['test_loss']
