...
构建描述子满足以下特性：
                        ...
                        置换不变性
input: coord.npy
output: descriptor.npy

'''

def simple_descriptor_builder(coords):
    # 取出各个原子的坐标
...
data = np.load('MD/process_data/coord.npy')
desc = simple_descriptor_builder(data)

output_dir = 'MD/process_data'
np.save(os.path.join(output_dir, "descriptor.npy"), desc.astype(np.float32))

print(desc.shape)  # (1000, 3)
print(desc)
print(f"saved descriptor.npy to '{output_dir}/'")
